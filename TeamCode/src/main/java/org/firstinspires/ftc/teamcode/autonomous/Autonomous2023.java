...
public class Autonomous2023 extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        DriveForward(config.drive_speed,43.5,10); // move to the cone

        DetectSignalSymbol(5); // detect the beacon color

        DriveForward(config.drive_speed,20,10); // Center the robot in position
    ...
    public void DriveForward(double speed, double distance, double timeoutS) {
        ...
        if (opModeIsActive()) {
            ...
            while (opModeIsActive() && (config.runtime.seconds() < timeoutS)) {
                ...
                telemetry.addData("Currently at", " at %d :%d", config.leftDrive.getCurrentPosition(), config.rightDrive.getCurrentPosition());
                telemetry.update();
            }
    ...
    public void DriveBackward(double speed, double distance, double timeoutS) {
        ...
        if (opModeIsActive()) {
            ...
            while (opModeIsActive() && (config.runtime.seconds() < timeoutS)) {
                ...
                telemetry.addData("Running to", " %d :%d", config.right_1_wheel_target, config.left_1_wheel_target);
                telemetry.addData("Currently at", " at %d :%d", config.leftDrive.getCurrentPosition(), config.rightDrive.getCurrentPosition());
                telemetry.update();
            }
    ...
    public void linslide_up(double speed, double distance, double timeoutS) {
        ...
        if (opModeIsActive()) {
            ...
            while (opModeIsActive() && (config.runtime.seconds() < timeoutS)) {
                ...
                telemetry.addData("Running to", " %d :%d", config.slide_1_target, config.slide_2_target);
                telemetry.addData("Currently at", " at %d :%d", config.linslide_right.getCurrentPosition(), config.linslide_left.getCurrentPosition());
            }
            // Stop all motion;
            config.LiftBrake();
        }
    ...
    public void DetectSignalSymbol(int number_of_color_tests){
        ...
        double temp_col_red = 0, temp_col_green=0, temp_col_blue = 0;

        for (int a = 0; a < number_of_color_tests; a++){
            config.delay(0.5); // Sleep for 1/4 second before trying again
        ...
        double col_red = temp_col_red / number_of_color_tests;
        double col_green = temp_col_green / number_of_color_tests;
        double col_blue = temp_col_blue / number_of_color_tests;

        if((col_green > col_red) && (col_green > col_blue)){
...
//
//        telemetry.addData("Colors_norm", "r: %f, g: %f, b: %f", temp_col_red, temp_col_green, temp_col_blue);
//        telemetry.addData("Colors_avg", "r: %f, g: %f, b: %f", col_red, col_green, col_blue);
